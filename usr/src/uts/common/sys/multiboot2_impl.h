...
/* MB2 tag browsing */
#define	MB2_FIRST_TAG(mbi)	(&(mbi)->tags[0])
#define	MB2_NEXT_TAG(tag)	\
	(((tag)->type == MULTIBOOT_TAG_TYPE_END)? NULL : \
	    ((multiboot_tag_t *)P2ROUNDUP((uintptr_t)((char *)(tag) + \
	    (tag)->size), MULTIBOOT_TAG_ALIGN)))

extern void *dboot_multiboot2_find_tag(multiboot2_info_header_t *, uint32_t);
extern int dboot_multiboot2_iterate(multiboot2_info_header_t *, void *);
...
extern int dboot_multiboot2_basicmeminfo(multiboot2_info_header_t *,
    uint32_t *, uint32_t *);
...
extern uint32_t dboot_multiboot2_mmap_get_type(multiboot2_info_header_t *,
    multiboot_tag_mmap_t *, int);
extern int dboot_multiboot2_mmap_entries(multiboot2_info_header_t *,
