...
{
	multiboot_header_tag_t *tagp = mbh2->tags;
	multiboot_header_tag_address_t *mbaddr = NULL;
	multiboot_header_tag_entry_address_t *mbentry = NULL;

	/*
	 * loop while we get end TAG or we have both tags.
	 */
	while (tagp->type != MULTIBOOT_HEADER_TAG_END &&
		...
		tagp = (multiboot_header_tag_t *)
		    ((uint32_t *)tagp +
		    P2ROUNDUP(tagp->size, MULTIBOOT_TAG_ALIGN) / 4);
	}
	...
	/* patch it */
	mbaddr->load_addr = ptload_start - ptload_offset;
	mbaddr->header_addr = mbaddr->load_addr + file_offset;
	mbentry->entry_addr = ptload_start;
...
patch64(Elf64_Ehdr *eh)
{
	 ...
	 * note we dont require it to be present.
	 */
	ndx = 0;
	for (mem2 = 0;
